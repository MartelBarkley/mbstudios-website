...
function initTypewriter() {
  ...
  // Helper: type text into an element, call onDone when finished
  ...
  // Sequence start — small delay for fonts/layout to settle
  setTimeout(() => {

    // 1. Type headline line 1
    type(line1, 'We Build Digital', 52, () => {

      // 2. Pause then type line 2
      setTimeout(() => {
        type(line2, 'Experiences That', 52, () => {

          // 3. Pause then type line 3
          setTimeout(() => {
            type(line3, 'Command Attention.', 52, () => {

              // 4. Apply accent styling to "Command"
              line3.innerHTML = '<em>Command</em> Attention.';

              // 5. Fade in sub + CTA buttons
              if (heroSub)     fadeIn(heroSub,     200);
              if (heroActions) fadeIn(heroActions,  450);
            });
          }, 120);
        });
      }, 120);
